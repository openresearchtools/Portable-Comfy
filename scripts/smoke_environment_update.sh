#!/usr/bin/env bash
# Install the downloaded environment bundle into a downloaded first-install tree.
...
[[ -f "$full_archive" && -f "$environment_archive" ]] \
  || die "usage: $0 FULL_ARCHIVE ENVIRONMENT_ARCHIVE [--timeout SECONDS]"
full_archive="$(absolute_path "$full_archive")"
environment_archive="$(absolute_path "$environment_archive")"
...
python3 "$SCRIPT_DIR/smoke_environment_update.py" \
  "$root" "$environment_archive" --timeout "$timeout_seconds"
log "transactional environment-update smoke passed"
